...
async def test_graph_get_user_happy(mocker, secrets):
    mocker.patch("activities.graph_users.load_tenant_secrets", return_value=secrets)
    ...
    user = await graph_get_user("t1", "john@example.com")
...
async def test_graph_get_user_404(mocker, secrets):
    ...
    assert await graph_get_user("t1", "none@example.com") is None
...
async def test_graph_create_user_happy(mocker, secrets, user_data):
    ...
    created = await graph_create_user("t1", user_data)
...
async def test_graph_update_user_happy(mocker, secrets):
    ...
    assert await graph_update_user("t1", "u1", {"department": "Finance"}) is True
...
async def test_graph_delete_user_paths(mocker, secrets):
    ...
    assert await graph_delete_user("t1", "missing") is False
...
async def test_graph_revoke_sessions_happy(mocker, secrets):
    ...
    assert await graph_revoke_sessions("t1", "u1") is True
...
async def test_graph_assign_and_reset_password_happy(mocker, secrets):
    ...
    assert await graph_assign_license("t1", "u1", "sku") is True
    assert await graph_reset_password("t1", "u1", "TempP@ss1234") is True
