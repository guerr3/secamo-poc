...
@pytest.mark.asyncio
@pytest.mark.parametrize("provider", STUB_PROVIDERS)
async def test_stub_connectors_reject_identity_risk_actions(provider: str, secrets: TenantSecrets):
    connector = get_connector(provider=provider, tenant_id="tenant-1", secrets=secrets)

    with pytest.raises(ConnectorUnsupportedActionError):
        await connector.execute_action("get_identity_risk", {"lookup_key": "user@example.com"})

    with pytest.raises(ConnectorUnsupportedActionError):
        await connector.execute_action("list_risky_users", {"min_risk_level": "medium"})
