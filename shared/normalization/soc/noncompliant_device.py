...
def normalize_noncompliant_device_case(envelope: Envelope, *, auto_remediate: bool) -> SecurityCaseInput:
    """Normalize a defender.security_signal(noncompliant_device) envelope into SecurityCaseInput."""

    payload = envelope.payload
    if not isinstance(payload, DefenderSecuritySignalEvent) or payload.provider_event_type != "noncompliant_device":
        raise ValueError(
            "normalize_noncompliant_device_case requires defender.security_signal payload with "
