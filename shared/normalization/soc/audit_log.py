...
def normalize_audit_log_case(envelope: Envelope, *, auto_remediate: bool) -> SecurityCaseInput:
    """Normalize a defender.security_signal(audit_log) envelope into SecurityCaseInput."""

    payload = envelope.payload
    if not isinstance(payload, DefenderSecuritySignalEvent) or payload.provider_event_type != "audit_log":
        raise ValueError(
            "normalize_audit_log_case requires defender.security_signal payload with provider_event_type=audit_log"
        )
