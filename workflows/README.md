...
> This folder defines parent Temporal workflows that orchestrate IAM and SOC automation, ingress routing, subscription reconciliation, and polling loops.
...
| `defender_alert_enrichment.py` | WF-02 parent workflow for Defender alert enrichment, optional threat intel, ticket creation, notify, and audit. | Started by Graph ingress router for alert resources. |
| `graph_ingress_router.py` | Routes validated Graph notifications to child workflows based on resource mapping. | Started by `graph_ingress/dispatcher.py`. |
...
| `impossible_travel.py` | WF-05 parent workflow for risky sign-in triage, ticketing, HITL, and incident response. | Started by Graph ingress router for sign-in/risky-user resources. |
...
- Route additions must stay consistent with mapper logic in `shared/models/mappers.py` and ingress router expectations.
