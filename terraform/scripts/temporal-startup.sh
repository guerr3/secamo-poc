...
services:
  ...
  secamo-graph-ingress:
    container_name: secamo-graph-ingress
    build:
      context: /opt/secamo-poc
      dockerfile: Dockerfile
    depends_on:
    ...
    environment:
      ...
      - ENVIRONMENT=${environment}
      - AWS_REGION=${region}
      - GRAPH_INGRESS_HOST=0.0.0.0
      - GRAPH_INGRESS_PORT=8081
    entrypoint: ["python", "-m", "graph_ingress.launcher"]
    healthcheck:
      test: ["CMD", "python", "-c", "import urllib.request; urllib.request.urlopen('http://localhost:8081/healthz', timeout=3)"]
      interval: 30s
      timeout: 5s
      start_period: 10s
      retries: 3
