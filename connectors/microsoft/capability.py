...
class _MicrosoftCapabilityConnector(BaseConnector):
    ...
    def __init__(
        ...
        *,
        transport: MicrosoftApiTransport,
    ) -> None:
        super().__init__(tenant_id=tenant_id, secrets=secrets)
    ...
    async def _request_graph(
        ...
        *,
        ...
        return await self._transport.request_graph(method, url, params=params, json=json)

    async def _request_defender(
        self,
        method: str,
        url: str,
        *,
        ...
        return await self._transport.request_defender(method, url, params=params, json=json)
    ...
    def _signal_title(
        ...
        *,
        ...
        # For risky_users, prefer userDisplayName over displayName to avoid
        # returning the risk-detection display name instead of the user's name.
        if resource_type == "entra_risky_users":
        ...
        *,
        ...
        return DefenderSecuritySignalEvent(
            ...
            vendor_extensions={
                ...
                "user_principal_name": VendorExtension(source=self.provider, value=user_principal_name),
    ...
    async def health_check(self) -> dict[str, Any]:
        response = await self._request_graph(
            "GET",
...
class MicrosoftDefenderEDRConnector(_MicrosoftCapabilityConnector):
    @property
    def provider(self) -> str:
    ...
    async def fetch_events(self, query: dict[str, Any]) -> list[Envelope]:
        ...
        while next_url:
            ...
            response = await self._request_graph("GET", next_url, params=next_params)
    ...
    async def _fetch_user_alerts(self, *, user_email: str, payload: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        response = await self._request_graph("GET", url, params=params)
        body = response.json()
        alerts = [
            item
    ...
    async def execute_action(self, action: str, payload: dict[str, Any]) -> dict[str, Any]:
        if action == "enrich_alert_context":
            ...
            if alert_id:
                ...
                try:
                    alert_response = await self._request_graph("GET", alert_url)
            ...
            if user_email:
                ...
                try:
                    user_response = await self._request_graph("GET", user_url)
            ...
            if device_id:
                ...
                try:
                    device_response = await self._request_graph("GET", device_url)
        ...
        if action in {"list_user_alerts", "get_user_alerts"}:
            ...
            alerts = await self._fetch_user_alerts(user_email=user_email, payload=payload)
            return {
                "success": True,
                "provider": self.provider,
        ...
        if action == "run_antivirus_scan":
            ...
            try:
                response = await self._request_defender(
                    "POST",
        ...
        if action == "get_device_context":
            ...
            try:
                response = await self._request_defender("GET", url)
            except ConnectorPermanentError as exc:
                if self._connector_error_status(exc) == 404:
                    return {
                        "provider": self.provider,
        ...
        if action == "isolate_device":
            ...
            try:
                response = await self._request_defender("POST", url, json=body)
            except ConnectorPermanentError as exc:
                if self._connector_error_status(exc) == 404:
                    return {
                        "success": True,
...
class MicrosoftGraphIdentityConnector(_MicrosoftCapabilityConnector):
    @property
    def provider(self) -> str:
    ...
    async def execute_action(self, action: str, payload: dict[str, Any]) -> dict[str, Any]:
        if action == "get_user":
            ...
            try:
                response = await self._request_graph("GET", url)
            except ConnectorPermanentError as exc:
                if self._connector_error_status(exc) == 404:
        ...
        if action == "list_risky_users":
            ...
            if len(filter_parts) > 1 and lookup_key and "@" in lookup_key:
                query_filter = " and ".join(["(" + " or ".join(filter_parts[:-1]) + ")", filter_parts[-1]])
            else:
                query_filter = " or ".join(filter_parts)
...
class MicrosoftGraphSubscriptionConnector(_MicrosoftCapabilityConnector):
    @property
    def provider(self) -> str:
        return "microsoft_graph"
    ...
    async def execute_action(self, action: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "renew_subscription":
            ...
            if not resource_hint:
                subscription_response = await self._request_graph(
                    "GET",
